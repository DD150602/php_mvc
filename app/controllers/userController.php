  ...
  public function actualizarUsuarioControlador()
  {
    ...
    if ($admin_usuario == "" || $admin_clave == "") {
      $alerta = array(
        "tipo" => "simple",
        "titulo" => "Campos obligatorios sin rellenar",
        "texto" => "No has llenado todos los campos que son obligatorios, los cuales corresponden a su usuario y clave",
        "icono" => "error"
      );
      return json_encode($alerta);
      exit();
    ...
    if ($check_admin->rowCount() == 1) {
      ...
      $alerta = array(
        ...
        "texto" => "Usuario o clave de administrador incorrectos BBBBB",
        "icono" => "error"
      );
      return json_encode($alerta);
      exit();
  ...
  public function actualizarFotoUsuarioControlador()
  {
    ...
    if ($this->actualizarDatos("usuario", $usuario_datos_up, $condicion)) {
      ...
      $alerta = array(
        ...
        "texto" => "No hemos podido actualizar algunos datos del usuario " . $datos['usuario_nombre'] . " " . $datos['usuario_apellido'] . " sin embargo la foto se actualizo correctamente",
        "icono" => "warning"
      );
    }
  ...
  public function eliminarFotoUsuarioControlador()
  {
    ...
    if (is_file($img_dir . $datos['usuario_foto'])) {
      ...
      if (!unlink($img_dir . $datos['usuario_foto'])) {
        $alerta = array(
          "tipo" => "simple",
          "titulo" => "No se ha podido eliminar la imagen",
          "texto" => "No se ha podido borrar la imagen del usuario en el sistema",
          "icono" => "error"
        );
        return json_encode($alerta);
        exit();
      ...
      $alerta = array(
        ...
        "texto" => "No se ha encontrado la foto en el sistema",
        "icono" => "error"
      );
      return json_encode($alerta);
      exit();
    ...
    if ($this->actualizarDatos("usuario", $usuario_datos_up, $condicion)) {
      ...
      $alerta = [
        ...
        "texto" => "La foto del usuario " . $datos['usuario_nombre'] . " " . $datos['usuario_apellido'] . " se eliminio correctamente",
        "icono" => "success"
      ];
    } else {
      $alerta = array(
        "tipo" => "recargar",
        "titulo" => "Foto eliminada",
        "texto" => "No hemos podido actualizar algunos datos del usuario " . $datos['usuario_nombre'] . " " . $datos['usuario_apellido'] . " sin embargo la foto se elimino correctamente",
        "icono" => "warning"
      );
    }
    return json_encode($alerta);
